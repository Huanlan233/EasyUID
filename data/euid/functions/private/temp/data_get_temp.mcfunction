...
execute if score @s euid.uid = @s euid.temp run data modify storage euid:return Output set from storage euid:data_temp Input[-1]
# 清空 temp
execute if score @s euid.uid = @s euid.temp run data remove storage euid:data_temp Input
# 递归执行
